...
--AS
DECLARE @Database_Name NVARCHAR(MAX) = 'DBSV2_7July2014',
@SchemaName_n_TableName NVARCHAR(Max) = 'dbo.Bank',
@Date_From DATETIME='1900/01/01',
@Date_To DATETIME ='9999/12/31'
...
DECLARE @TableName VARCHAR(50) = RIGHT(@SchemaName_n_TableName,LEN(@SchemaName_n_TableName)-4),
@ColumnName VARCHAR(MAX),@on nvarchar(MAX), @off nvarchar(MAX)

set @ColumnName =(SELECT t.Column_name +',' FROM information_schema.COLUMNs t WHERE t.TABLE_NAME = @TableName FOR XML PATH(''))
select @ColumnName='Insert into dbo.'+@TableName+' ('+Left(@ColumnName,len(@ColumnName)-1)+')'
SET @sql = 'SELECT ' + @FieldName  + 'into #Temp FROM #temp_Data PIVOT (Min([FieldValue]) FOR FieldName IN (' + @FieldName  + ')) AS pvt
 '+@ColumnName+' SELECT * FROM #Temp'
set @on =  'set IDENTITY_INSERT '+ @SchemaName_n_TableName+' ON'
SET @off = 'set IDENTITY_INSERT '+@SchemaName_n_TableName+' OFF'
--print(@a)
--PRINT(@sql)
...
DROP TABLE #temp_Data
 --SELECT DISTINCT FieldName+',' FROM #temp_Data
--DROP TABLE #temp_Data
-- INSERT INTO Bank
--GO
