...
cc.[COLD CHAIN - STORAGE], cc.[COLD CHAIN - TRANSIT SALES],
cc.[COLD CHAIN - TRANSIT INTER BR],
...
SELECT Productid,
       --MAX([PROD SSR SEQ]) AS [PROD SSR SEQ],
       MAX([COLD CHAIN - STORAGE]) AS [COLD CHAIN - STORAGE],
       MAX([COLD CHAIN - TRANSIT SALES]) AS [COLD CHAIN - TRANSIT SALES],
       MAX([COLD CHAIN - TRANSIT INTER BR]) AS [COLD CHAIN - TRANSIT INTER BR]
       --MAX([IS HEAVY WT CARTON?]) AS [IS HEAVY WT CARTON?] 
FROM   uv_ProductProfile upp
       PIVOT(
           MAX(upp.value) FOR upp.name IN ([PROD SSR SEQ], 
                                          [COLD CHAIN - STORAGE], 
                                          [COLD CHAIN - TRANSIT SALES], 
                                          [COLD CHAIN - TRANSIT INTER BR], 
                                          [IS HEAVY WT CARTON?])
       ) AS ValuePerProfile
GROUP BY
       Productid)
       CC ON cc.Productid = b.[Product Code]
GROUP BY 
